...
interface SyLock {
   ...
   */
  default
  <R> R lock(@NotNull LockType lockType, @NotNull trySupplier<R> run)
  { return lock(lockType, run, null, null); }
   ...
   * @see tryRunnable
   * @see LockType
   * @see #lock(LockType, tryRunnable, Consumer, Runnable)
   * @since 0.1.0
   */
  default
  void lock(@NotNull LockType lockType, @NotNull tryRunnable run)
  { lock(lockType, run, null, null); }
   ...
   * @see tryRunnable
   * @see LockType
   * @see #trylock(LockType, tryRunnable, Runnable)
   * @since 0.1.0
   */
  default
   ...
   */
  default
  <R> R read(@NotNull trySupplier<R> run)
  { return lock(LockType.READ, run, null, null); }
  ...
  { lock(LockType.READ, run, null, null); }
   ...
   */
  default
  <R> R write(@NotNull trySupplier<R> run)
  { return lock(LockType.WRITE, run, null, null); }
  ...
  { lock(LockType.WRITE, run, null, null); }
   ...
   */
  default
  <R> R tryread(@NotNull trySupplier<R> run) throws Exception
  { return trylock(LockType.READ, run, null); }
  ...
  { trylock(LockType.READ, run, null); }
   ...
   */
  default
  <R> R trywrite(@NotNull trySupplier<R> run) throws Exception
  { return trylock(LockType.WRITE, run, null); }
  ...
  { trylock(LockType.WRITE, run, null); }

  /**
   * 尝试使用读锁执行指定回调
   * <p>
   * 调用读锁执行，运行时自带try-finally块，通过两个回调参数插入不同的块中执行，遇到异常会抛出<br/>
   * 所有回调均在并发域内执行
   *
   * @param run    带返回的回调
   * @param finaby 进入finally块后的回调，传入前一个回调的返回值，遇到异常传入{@code null}
   ...
   */
  default
  <R> R tryread(@NotNull trySupplier<R> run, @Nullable Function<R, R> finaby) throws Exception
   ...
   */
  default
  void tryread(@NotNull tryRunnable run, @Nullable Runnable finaby) throws Exception
  { trylock(LockType.READ, run, finaby); }

  /**
   * 尝试使用写锁执行指定回调
   * <p>
   * 调用写锁执行，运行时自带try-finally块，通过两个回调参数插入不同的块中执行，遇到异常会抛出<br/>
   * 所有回调均在并发域内执行
   *
   * @param run    带返回的回调
   * @param finaby 进入finally块后的回调，传入前一个回调的返回值，遇到异常传入{@code null}
   ...
   */
  default
  <R> R trywrite(@NotNull trySupplier<R> run, @Nullable Function<R, R> finaby) throws Exception
   ...
   */
  default
  void trywrite(@NotNull tryRunnable run, @Nullable Runnable finaby) throws Exception
